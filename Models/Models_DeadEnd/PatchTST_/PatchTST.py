...
from Models.Models_DeadEnd.PatchTST_.PatchTST_backbone import PatchTST_backbone
from Models.Models_DeadEnd.PatchTST_.PatchTST_layers import series_decomp
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        # load parameters
        c_in = configs.num_series
        context_window = configs.past_history
    ...
    def forward(self, x,x_dec=None):  # x: [Batch, Input length, Channel]
        if self.decomposition:
